...
var (
	...
	switchboardAddressPlaceholder = regexp.MustCompile(`"[^"\s].*/FungibleTokenSwitchboard.cdc"`)
)
...
type Contracts struct {
	SwitchboardAddress flow.Address
	SwitchboardSigner  crypto.Signer
}

func switchboardDeployContracts(t *testing.T, b *emulator.Blockchain) Contracts {
	accountKeys := test.AccountKeyGenerator()

	switchboardAccountKey, switchboardSigner := accountKeys.NewWithSigner()
	switchboardCode := loadSwitchboard(ftAddress)

	switchboardAddress, err := b.CreateAccount(
		[]*flow.AccountKey{switchboardAccountKey},
		nil,
	)
	require.NoError(t, err)

	fundAccount(t, b, switchboardAddress, defaultAccountFunding)
	...
	signAndSubmit(
		t, b, tx,
		[]flow.Address{b.ServiceKey().Address, switchboardAddress},
		[]crypto.Signer{b.ServiceKey().Signer(), switchboardSigner},
		false,
	)
...
func installSwitchboard(
	...
	tx := flow.NewTransaction().
		SetScript(switchboardGenerateInstallSwitchboardScript(contracts)).
		SetGasLimit(100).
		SetProposalKey(b.ServiceKey().Address, b.ServiceKey().Index, b.ServiceKey().SequenceNumber).
		SetPayer(b.ServiceKey().Address).
...
func setupAccount(
	...
	installSwitchboard(t, b, address, signer, contracts)
